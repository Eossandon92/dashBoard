...
const Expense = () => {
    ...
    // --- FUNCIÓN GENÉRICA PARA CAMBIAR ESTADO (Anular, Aprobar, etc) ---
                    ...
                    <div className="p-6 h-full flex flex-col overflow-hidden">
                        <div className="w-full rounded-md border bg-background overflow-hidden flex flex-col flex-1 shadow-sm">
                            <div className="flex items-center justify-between gap-4 border-b p-4" >
                                <h3 className="text-xl font-bold whitespace-nowrap">Historial de Gastos</h3>
                                <Input className="w-[180px] text-sm" placeholder="Buscar…" value={search} onChange={(e) => setSearch(e.target.value)} />
                            </div>
                                    ...
                                    <TableHeader>
                                        <TableRow className="sticky top-0 z-20 bg-slate-50 shadow-sm">
                                            {EXPENSE_COLUMNS.map((c) => (
                                                <TableHead key={c.key} className="font-semibold text-slate-600 tracking-wide">{c.label}</TableHead>
                                            ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-800">Detalle del Gasto</h3>
                                <p className="text-sm text-slate-500">Documento N° {selectedExpense.document_number || 'S/N'}</p>
                            </div>
                            <button onClick={() => setIsDetailOpen(false)} className="p-2 hover:bg-slate-200 rounded-full transition-colors"><X className="h-5 w-5 text-slate-500" /></button>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-6">
                                    <div className="flex justify-between items-center">
                                        <span className={`px-3 py-1 rounded-full text-xs font-medium border ${selectedExpense.status === 'Pagado' ? 'bg-green-50 text-green-700 border-green-200' : selectedExpense.status === 'Anulado' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'}`}>{selectedExpense.status}</span>
                                                ...
                                                <div key={index} className="group relative rounded-lg border border-slate-200 overflow-hidden bg-slate-50 hover:shadow-md transition-all">
                                                    {doc.url.match(/\.(jpeg|jpg|png|gif|webp)$/i) ? (
                                                        <a href={doc.url} target="_blank" rel="noopener noreferrer">
                                                            ...
                                                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100"><span className="bg-white/90 text-xs py-1 px-3 rounded-full shadow-sm font-medium">Ver Pantalla Completa</span></div>
                                                        </a>
                                                    ) : (
                                                        <div className="p-6 flex flex-col items-center justify-center text-slate-400"><FileText className="h-12 w-12 mb-2" /><a href={doc.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm font-medium">Ver Documento ({doc.type || 'Archivo'})</a></div>
                                                    )}
                                                </div>
                                            ))
                                        ) : (
                        ...
                        <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end">
                            <button onClick={() => setIsDetailOpen(false)} className="px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-md hover:bg-slate-800 transition-colors">Cerrar</button>
                        </div>
                    </div>
                </div>
            )}
